...
- 按 **13 大类 + workflow-catalog.yaml 的 7 阶段 pipeline** 双维度组织
...
| Team Orchestration | **9 个 team-* 共同模式合并精读** | **01b P0 重灾区，以共同模式 + 9 个适配表覆盖** |
...
> **覆盖度说明**：本册精读 11 个 skill / 共 72 个，**精读率约 15%**。未精读的 61 个 skill 仅在分类概览表里列出字段要点（无 step-by-step 拆解、无产出文件细节、无 frontmatter 全量），其中部分 MVS 必含 skill（如 `/story-done` `/sprint-plan` `/qa-plan` `/smoke-check` `/setup-engine` `/retrospective` `/scope-check` 等）当前仅 1 行表格描述。读者在做移植决策时如需这些 skill 的执行细节，请直接打开 `.claude/skills/[name]/SKILL.md` 原文，本册不再二次复述。
...
根据 README.md 和 `workflow-catalog.yaml`，72 个 skill 可分为以下 13 大类（基于**工作阶段**而非技术实现；其中类别 5 与 7 存在 2 项交叉归类，去重后实际为 72）：
...
| 串行单个 | 23 | 32% |
| 串行多个（2-4 个） | 17 | 24% |
| **并行多个（⚠️ CodeBuddy 瓶颈）** | **15** | **21%**（按 §940 列项口径，含 art-bible/asset-spec 的 full 模式） |

> **口径说明**：art-bible 与 asset-spec 在默认模式下属"串行多个"，仅 `--full` 模式触发并行。本表的"串行多个 17"含其默认形态，"并行多个 15"含其 full 形态——同一 skill 因模式不同被同时统计在两类中，导致 18+23+17+15 = 73 > 72。这是 CCGS 设计上的双形态使然，移植决策时需根据实际启用模式判断。

**15 个并行多个的 skill** = `review-all-gdds` + `gate-check` + `art-bible` (full) + `asset-spec` (full) + 9 个 team-* + `ux-design` + `ux-review`（1+1+1+1+9+1+1=15）。**这 15 个是移植改造的重点**。
...
**15 个"并行多个"** = 必须改造 → 完整档或舍弃（含 art-bible/asset-spec 的 full 模式，详见 §938 口径说明）

### 洞察 2：团队 orchestration 是 CodeBuddy 最大痛点

9 个 `team-*` skill 全部是并行多个，占 15 个并行 skill 的 **60%**（9/15）。极简档必须全部跳过；标准档只推荐保留 3-4 个核心；完整档需要接受 3-6 倍延迟。
...
*下一步：04_Hooks + Rules + Templates（12 hook / 11 rule / 38 template 全解析）。建议审阅 03 册整体后继续。*
