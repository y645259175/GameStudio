...
| 03 | 72 skill → 15 个并行多个（必改造）+ 9 个 team-* 是 P0 重灾区 |
...
**严重（P0-P1）—— 影响整体架构 2 项**：
- 无并行 subagent —— 影响 15 个 skill 的设计（9 个 team-* + review-all-gdds + gate-check + art-bible(full) + asset-spec(full) + ux-design + ux-review）
...
- **性能**（并行 spawn 损失 → 15 个 skill 延迟 3-6 倍）
...
> **MVS 总数公式**：Agents 9 + Skills 20 + Hooks 4 + Rules 4 + Templates 9 = **46**（取各列范围最小值口径；按最大值口径上限为 10+20+4+5+9 = 48，但本报告统一以 46 作为单一数字，避免多口径混淆）。

### CCGS → MVS 减幅核算

182 → 46 即 (182-46)/182 ≈ 75%（原表写"~74%"按 46-48 区间四舍五入，本次统一为 46 后修正为 ~75%）。
...
| Discovery（3，**可选**） | /adopt /consistency-check /quick-design | 灵活补充（按项目需要启用） |

**说明**：MVS **不含 9 个 team-***（极简档全砍） + **不含完整架构 4 skill**（架构决策由 lead-programmer 直接做，跳过 ADR 流程） + **不含 Release 5 skill**（Game Jam 阶段无需）。

> **20 + 3 可选口径**：MVS 必备 17 个（Onboarding 4 + Game Design 3 + Stories 3 + Reviews 1 + QA 3 + Production 2 + Creative 1 = 17）+ Discovery 类 3 个可选 = 共 20 个 skill 名义计入 MVS（与 §171 "Skills MVS = 20" 对齐）。其中 Discovery 三项（adopt / consistency-check / quick-design）以及 Reviews 类的 design-review 与 Creative 类的 prototype 在 03 册 §953 中均标 "可选" —— 移植时建议至少安装 17 个必备项，可选项按用户实际需要启用。
...
| 3 | **核心工作流（Core Workflows）** | 20 个 MVS skill | ~46（与 §171 MVS 总数对齐） | 1-2 天 | "工作室能跑通完整的设计→开发→测试闭环" |
...
> **口径说明**：Phase 1/2 的累计数（~18 / ~28）包含 CODEBUDDY.md / workflow-catalog.yaml / settings.json 等"框架文件"，与 §171 MVS 总数（5 类资产 = Agents/Skills/Hooks/Rules/Templates）的 46 口径相差 1-2 项框架文件。Phase 3 累计已统一为 46，与 §171 口径一致；Phase 1/2 数字保留含框架文件口径作为路线图阶段产物的真实计数。
...
- [ ] **关键**：15 个并行多个 skill 全部改为串行（虽然 MVS 不含 9 team-*，但仍需关注 review-all-gdds 等）
- [ ] 测试 MVS 验证标准的 6 个动作
...
Phase 2 (角色)        ─┼─→  +10 = ~28 / 1-2 天
                       │
Phase 3 (MVS)         ─┴─→  +20 = ~46 / 1-2 天      ← MVS 部署完成
                       │                              ← 可开始实际游戏开发
...
> **清单基线说明（与 §171 MVS / §688 三档对比的口径差异）**：本节清单 A/B/C 是基于 Q1-Q5 自评结果生成的**自定义起步清单**，与第 3 章 MVS（46）/ §688 标准档（~107）/ 完整档（~164）**不直接对应**：
> 
> | 清单 | 含义 | 对应档位（参考） | 总资产 |
> | --- | --- | --- | --- |
> | A | Game Jam / 周末项目最小起步集（仅核心闭环，比 MVS 更精简） | 比 MVS 还窄一档 | 24 |
> | B | 标准独立工作室起步集（接近 §688 标准档但起步更轻） | ≈ 标准档起步 | 107 |
> | C | 完整商业项目起步集（接近 §688 完整档去掉 sound-designer / community-manager 等） | ≈ 完整档起步 | 164 |
> 
> **建议用法**：A 适合 Game Jam 临时项目（从 24 起，需要时再补到 MVS 46）；B/C 适合直接对应档位起步。如需严格对齐 §688 三档对比表，请以 §688 数字（46 / ~107 / ~164）为准；本节清单数字与 §688 一致（24 / 107 / 164），均反映 04 册修订后的 hook 数（标准档 7 / 完整档 8）。
...
Hooks: 7
  - 全部 8 个可移植事件中的 7 个（除 validate-skill-change 留待完整档）
...
2. **性能损失约 30%（仅限 15 个并行 skill），功能损失 < 10%（3 个单点机制）**
3. **从 MVS 起步（46 资产 / 1-2 天），按真实痛点渐进式扩展到完整档（约 164 资产 / 持续）**
