...
**标准档保留 = 7 个**（加 3 个 validate-*：commit / push / assets；validate-skill-change 留待完整档）  
**完整档保留 = 8 个**（加 validate-skill-change）

> **口径说明**：post-compact.sh 在 CodeBuddy 无对应事件 → 标准/完整档列为 ⚪（带代偿不计入实保留数）。"可移植"（事件层 8/12）≠"必保留"（档位层 4 / 7 / 8），二者不要混淆。
...
CodeBuddy 的事件机制不全，但**事件层 8/12 个 Hook 可移植**（上下文管理 + 质量校验）。损失的 3 个（log-agent / log-agent-stop / notify）和 1 个（post-compact）可代偿。

> **口径提示**："8/12 可移植"是事件层是否能注册的口径；档位层实际保留数为极简 4 / 标准 7 / 完整 8（详见 §327-328）。两口径不互通，引用时请注意。
...
|---|---|---|---|---|---|
| Hooks | 12 | 4 | 7 | 8 | 3 hook 不可移植（log-agent×2 + notify）+ 1 部分丢失（post-compact，标准/完整档列 ⚪ 表示带代偿不计入实保留数） |
| Rules | 11 | 4-5 | 8-9 | 11 | 路径作用域机制丢失，用 alwaysApply + 智能体请求代偿 |
| Templates | 38 | 9 | 27 | 38 | **零损失** |
| **合计** | **61** | **17-18** | **42-43** | **57** | 3 资产丢失，5%-15% 功能影响 |
