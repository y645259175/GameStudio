...
| # | 内容 | 优先级 | 验证 | 状态（2026-06-25） |
|---|---|---|---|---|
| M3-1 | 玩家能完成一次完整循环（宗门 → 历练 → 回宗 → 资源回流 → 修炼/建造）| P0 | 实玩 smoke | ✅ e2e_smoke step 5 PASS（6 节点历练 +900 灵石回流）|
| M3-2 | 5 件基本功能：主菜单 / 开始 / 结束 / 存档 / 读档 全可用 | P0 | 实玩 smoke | ✅ main_menu.tscn + game_over.tscn + GameManager + SaveService slot 0/99 双向 |
| M3-3 | 至少 10 种数据驱动事件可触发（GDD-03 §2.9.5 M3 20-30 个）| P0 | smoke | ✅ 14 事件 / 66 动作 / 6 选项（事件配置.xlsx）|
| M3-4 | 边际递减系统在第 3 月明显感知（GDD-01 §2.3 / GDD-04 §3.3 / GDD-06 §4）| P0 | 用户实玩反馈问卷 | ⏳ 等用户实玩反馈（diminishing_factor 已实装 GDD-04 §2.2）|
| M3-5 | UI 视觉达基础商业化标准 | P0 | art-director APPROVE | ✅ AD-PHASE-GATE PASS（13 张水墨写意资产 + 宣纸卡片 UI）|
| M3-6 | 主要立绘有 canonical key（门主 + 初始弟子 + 1-2 关键 NPC）| P0 | art-director gate | ✅ 6 anchors + master/disciple_male/disciple_female/disciple_recruit |
| M3-7 | 存档跨"假装的版本升级"测试通过（手工改 save_version 模拟）| P0 | 单元测试 | ✅ bootstrap_check #32 缺 chunk 降级测试 PASS |
| M3-8 | 1 张历练图（含 ~20-30 节点 + 中途撤离 + 终点撤离）| P0 | 配表 | ⚠️ 6 节点 DAG（三层池抽样），M3 简版（节点数低于 GDD 推荐 20-30，M4 扩）|
| M3-9 | 5 建筑 + 5 炼丹配方 + 招收（自动来投 + 主动）跑通 | P0 | 配表 + smoke | ✅ 5 建筑×3级 / 5 配方全可炼 / RecruitService 双模式 |
| M3-10 | 战斗（StatSimulator + 五行）+ 战利品落袋跑通 | P0 | smoke | ✅ 100 局：同境 47% / 高一境 100% / 低一境 0%（全命中 §5.2 目标）|
| M3-11 | 撤离 3 类（active / timeout / defeat）触发正确 | P0 | smoke | ✅ 4 类（active/timeout/defeat/cleared）e2e PASS |
| M3-12 | 数据驱动：所有平衡数值（GDD-06）全配表化（无硬编码）| P0 | review | ⚠️ CONDITIONAL：核心运行时数据 174 行已配表（建筑/事件/资源/境界/炼丹/buff），剩余 const 为公式系数 fallback（SALARY_BY_REALM / BASE_POWER / COUNTER），M4 计划全配表化 |
...
| 指标 | 目标 | 验证 | 状态 |
|---|---|---|---|
| 单局时长 | 前期 ~20 / 后期 ~60 分钟 | 实玩计时 | ⏳ 等用户实玩 |
| 单局境界进度 | 炼气递进 1-3 级 + 可能 1 次大突破 | 实玩 | ⏳ 等用户实玩 |
| 自给经济转负 | 5-8 月（不出门）| 配表跑数 + 实玩 | ⚠️ 实测 8 月 2000→1520（仍盈余），需上调月俸或下调起步资源（M4 标定）|
| 单次历练净收益 | 200-500 灵石 | smoke 实测 | ✅ e2e 实测 +900（含战利品；M3 偏富裕，M4 标定）|
| 战斗胜率 | 同境界 ~50% / 高 1 大境界 ~碾压 | smoke 100 局 | ✅ 47% / 100% / 0%（全命中）|
